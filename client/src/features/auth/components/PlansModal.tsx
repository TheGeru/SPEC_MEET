/**
 * PlansModal — Presentational Component
 *
 * Shows plan options after successful registration.
 *
 * Scope Rule: This modal is ONLY used inside RegisterForm.
 * Previously lived in components/plans/PlansModal.tsx — a SCOPE VIOLATION
 * since no other feature uses it.
 *
          ...
          <div className="px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {PLANS.map((plan) => (
                ...
                >
                  {plan.popular && (
                    <div className="absolute top-0 right-0 bg-purple-500 text-white text-xs font-bold px-3 py-1 transform translate-x-2 -translate-y-2 rounded-bl-lg rounded-tr-lg">
                      RECOMENDADO
                    </div>
                  )}
                  ...
                  <Link
                    to="/booking"
                    onClick={onClose}
                    className={`block w-full py-2 px-3 rounded-md shadow text-center text-sm font-medium ${
                      plan.popular
