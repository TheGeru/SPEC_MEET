...
import { createReservationRequest } from '../../api/reservation.api'; 
...
// ... (MANTÉN TU CÓDIGO DE stripePromise Y CheckoutForm EXACTAMENTE IGUAL) ...
...
const BookingPage: React.FC = () => {
  ...
  // Estado para guardar ID de la sala real
  ...
  // 1. CARGAR LA SALA REAL DESDE EL BACKEND AL INICIAR
  React.useEffect(() => {
    const fetchRoom = async () => {
      try {
        // Usamos tu ruta existente /rooms
        const response = await api.get('/rooms');
        const rooms = response.data;
        
        // Como dijiste que solo usas una sala, tomamos la primera
        if (Array.isArray(rooms) && rooms.length > 0) {
          setRoomId(rooms[0].id);
          console.log("Sala configurada:", rooms[0].name);
        }
      } catch (error) {
  ...
  // ... (MANTÉN TUS FUNCIONES getDailyTimeSlots, navigateMonth, etc. IGUAL) ...
  ...
  const getDailyTimeSlots = () => {
    ...
    // MOCK DATA para visualización (Para disponibilidad real se requiere otro endpoint)
  ...
  const calculateISODates = () => {
    ...
    // Crear fechas locales y convertirlas a ISO para el backend
  ...
  const handleNextStep = async () => {
    if (currentStep === 'date') {
      ...
      try {
        ...
        // Payload exacto que espera tu reservation_controller.ts
        ...
        // Llamada al Backend
        const response = await createReservationRequest(reservationPayload);
        
        // Tu controller devuelve { clientSecret, reservationId, ... }
        ...
        // Mostrar mensaje de error del backend (ej: "Conflicto de horario")
  ...
  // ... (EL RESTO DEL ARCHIVO PERMANECE IGUAL: handlePrevStep, renderCalendarView, etc.) ...
  // Solo asegúrate de copiar el resto de las funciones de renderizado tal cual las tenías.
