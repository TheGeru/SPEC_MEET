...
const PlansPage: React.FC = () => {
  ...
  return <div className="w-full min-h-screen bg-gradient-to-b from-black to-zinc-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-12">
          {plans.map(plan => <div key={plan.id} className={`bg-zinc-800 rounded-lg shadow-lg overflow-hidden border ${plan.popular ? 'border-purple-500' : 'border-zinc-700'} relative`}>
              {plan.popular && <div className="absolute top-0 right-0 bg-purple-500 text-white text-xs font-bold px-3 py-1 rounded-bl-lg">
                  MÁS POPULAR
                </div>}
              <div className="p-6">
                ...
                <Link to="/booking" className={`block w-full py-3 px-4 rounded-md shadow text-center font-medium ${plan.popular ? 'bg-purple-600 hover:bg-purple-700 text-white' : 'bg-zinc-700 hover:bg-zinc-600 text-white'}`}>
                  Reservar Ahora
                </Link>
              </div>
            </div>)}
